...
All energy quantities are in **kJ** throughout the simulation:
  ...
  - Energy model output : kJ/slot  (e.g. 0.55 kJ/slot = 550 J/slot)

The paper expresses energy arrival as J/slot (e.g. Fig 3a x-axis: 50–600 J/slot).
Those values are divided by 1000 when stored here.

  Paper Fig 3a sweep:  50–600 J/slot  →  0.05–0.60 kJ/slot here
  Paper Fig 3b baseline: 550 J/slot   →  0.55 kJ/slot here
  Diurnal peak (our ext): 800 J/slot  →  0.80 kJ/slot here
"""
...
class SimConfig:
    ...
    # (Paper reports J/slot; divide by 1000 for kJ/slot used here)
    #
    ...
    # This matches the paper's results (devices go offline at p=1.0)
    # -------------------------------------------------------------------------
